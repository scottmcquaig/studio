...
export default function MockDailyPromptPage() {
  ...
  const challenge: Challenge | undefined = useMemo(() => {
    ...
    // In a real app, you'd fetch the specific challenge for the user's active track.
    // For the mock, we'll just use the main list.
  ...
  if (!challenge) {
    return (
      <div className="flex flex-col min-h-screen bg-background text-foreground">
        ...
        <BottomNav activeTab="Daily" currentDay={CURRENT_CHALLENGE_DAY} isMock={true}/>
      </div>
    );
  }
  
  const navLinks = {
    prev: day > 1 ? `/mock/day/${day - 1}` : null,
    next: day < 30 ? `/mock/day/${day + 1}` : null,
  };
